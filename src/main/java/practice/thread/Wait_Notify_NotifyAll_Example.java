...
class MyRunnable3 extends Thread {
	...
	public void run() {
		...
		synchronized (this.myRunnable1) {
			...
			System.out.println(Thread.currentThread().getName() + " has notified waiting threads");
		}
...
public class Wait_Notify_NotifyAll_Example {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		t1.start();
		t2.start();
		Thread.sleep(1000); // Used to ensure that thread1 and thread2 starts before thread-3
		// because thread-1 and 2 calls wait(), while thread-3 calls notify or
		// notifyAll()
		t3.start();
